...
# If you have run this already, skip the block and import "survivalanalysis/tcga_brca_data_2.15.RDS"
{
  ...
  data <- GDCprepare(
    ...
    directory = "survivalanalysis", 
    save = TRUE, 
    save.filename = "survivalanalysis/tcga_skcm_data_2.15.RDS"
  )
}

##### NOTE: IMPORT "survivalanalysis/tcga_skcm_data_2.15.RDS" TO START FROM HERE!!! (generated by above code!)
##### NOTE: IMPORT "survivalanalysis/tcga_skcm_data_2.15.RDS" TO START FROM HERE!!! (generated by above code!)
##### NOTE: IMPORT "survivalanalysis/tcga_skcm_data_2.15.RDS" TO START FROM HERE!!! (generated by above code!)
##### NOTE: IMPORT "survivalanalysis/tcga_skcm_data_2.15.RDS" TO START FROM HERE!!! (generated by above code!)
...
# Using TCGA data to find prog genes and generate Kaplan-Meier survival plots
{
  ...
  # "survivaldf" = Patient clinical metadata for generating survival plots
  {
    ...
    data.subtype <- TCGAquery_subtype(tumor = "SKCM")
    
    # ALL_MorP = "All_Metastases"
    ALL_MorP = "All_Primaries"
    data.subtype <- data.subtype[data.subtype$ALL_PRIMARY_VS_METASTATIC == ALL_MorP,]
    ...
    write.csv(survivaldf,paste0(ALL_MorP,"_survivaldf.csv"))
  }
  ...
  #  and provide TPM high/low bins for each gene to "survivaldf"
  {
    ...
    write.table(survival_genes_better,paste0(ALL_MorP,"_Prognostic_better_outcome_genes_0.05p_1.2HR.csv"), sep=",", col.names = F, row.names = F, quote = F)
    write.table(survival_genes_worse,paste0(ALL_MorP,"_Prognostic_worse_outcome_genes_0.05p_1.2HR.csv"), sep=",", col.names = F, row.names = F, quote = F)
  }
  ...
  #   "survival-curve_<TAG>_STATS.csv" = coxph() stats for the "high" curve, including Hazard Ratio
  {
    ...
    palette = c("#f3766e", "#4472c4") # Order is (high, low)
      ...
      write_csv(genes,paste0(out_path, ALL_MorP,"_gene-list_",tag,".csv"))
      ...
      write.csv(t$table_body, paste0(out_path, ALL_MorP,"survival-curve_", tag, "_STATS.csv"))
      ...
      ggsave(filename=paste0(out_path, ALL_MorP,"survival-curve_", tag, ".pdf"), plot = p, device = "pdf", width = 6, height = 9)
    }
  }
