...
# If you have run this already, skip the block and import "survivalanalysis/tcga_brca_data_2.15.RDS"
{
  ...
  data <- GDCprepare(
    ...
    directory = "survivalanalysis", 
    save = TRUE, 
    save.filename = "survivalanalysis/tcga_hnsc_data.RDS"
  )
}

##### NOTE: IMPORT "survivalanalysis/tcga_hnsc_data.RDS" TO START FROM HERE!!! (generated by above code!)
##### NOTE: IMPORT "survivalanalysis/tcga_hnsc_data.RDS" TO START FROM HERE!!! (generated by above code!)
##### NOTE: IMPORT "survivalanalysis/tcga_hnsc_data.RDS" TO START FROM HERE!!! (generated by above code!)
##### NOTE: IMPORT "survivalanalysis/tcga_hnsc_data.RDS" TO START FROM HERE!!! (generated by above code!)
...
# Using TCGA data to find prog genes and generate Kaplan-Meier survival plots
{
  ...
  # "survivaldf" = Patient clinical metadata for generating survival plots
  {
    ...
    data.subtype <- TCGAquery_subtype(tumor = "HNSC")
    ...
    Clinical_data <- Clinical_data[Clinical_data$histological_type == "Head & Neck Squamous Cell Carcinoma",]
    ...
    write.csv(survivaldf,"survivaldf.csv")
  }
  ...
  #  and provide TPM high/low bins for each gene to "survivaldf"
  {
    ...
    write.table(survival_genes_better,"Prognostic_better_outcome_genes_0.05p_1.2HR.csv", sep=",", col.names = F, row.names = F, quote = F)
    write.table(survival_genes_worse,"Prognostic_worse_outcome_genes_0.05p_1.2HR.csv", sep=",", col.names = F, row.names = F, quote = F)
  }
